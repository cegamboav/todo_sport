...
const form = useForm({
  ...
  email: '',
  phone: '',
  grade_id: (props.defaultGradeId ?? '') as string | number,
  notes: '',
  access_username: '',
  access_password: '',
...
function submit() {
  form
    ...
    .post('/masters/professors');
}
</script>

<template>
  <FlashAlert />
  <h1 class="page-title">Nuevo profesor</h1>
  <div class="card">
    <div class="card-body">
      <form @submit.prevent="submit">
        <div class="form-grid">
          ...
          <p class="hint" style="grid-column: 1 / -1; margin: 0">
            Acceso al portal profesores (<code>/school/login</code>). No incluye panel administrativo.
          </p>
        </div>
        <div class="form-actions">
          <button type="submit" class="btn btn-primary btn-inline" :disabled="form.processing">
            {{ form.processing ? 'Guardando…' : 'Crear profesor' }}
          </button>
