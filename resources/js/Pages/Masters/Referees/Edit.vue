...
const form = useForm({
  first_name: props.referee.first_name,
  last_name: props.referee.last_name,
  email: props.referee.email ?? '',
  phone: props.referee.phone ?? '',
  grade_id: props.referee.grade_id ?? '',
  specialty: props.referee.specialty,
  notes: props.referee.notes ?? '',
  access_username: '',
  access_password: '',
...
function submit() {
  form
    ...
    .put(`/masters/referees/${props.referee.id}`);
}
</script>

<template>
  <FlashAlert />
  <h1 class="page-title">Editar árbitro</h1>
  <div class="card">
    <div class="card-body">
